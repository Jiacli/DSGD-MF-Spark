...
self_eval = True   # calculating avg. NZSL using spark
verbose = False    # printing debug info
...
def main(args):
    ...
    if self_eval:
        ...
        print 'inital reconstruction err:', sumSL / n

    # perform stochastic gradient descent
    m = 0
    for t in xrange(num_iter):
        for strata in strata_list:
            results = strata.mapPartitions(lambda x: updateWeights(x, W, H, m, params)).collect()
            # update weights
            for x in results:
                if x[0] == 'W':
                    W[x[1]] = x[2]
        ...
        if self_eval:
            sumSL, n = V.map(lambda x: evaluation(x, W, H)).reduce(lambda x,y: (x[0]+y[0], x[1]+y[1]))
            print 'reconstruction err:', sumSL / n
...
def parse_line(record):
    ...
    return int(uid)-1, int(mid)-1, int(rating)
...
def updateWeights(iterator, W, H, m, params):
